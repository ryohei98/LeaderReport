...
function createLatest(sheet) {
  ...
  var row = sheet.getRange(lastRow, 1, 1, lastCol).getValues();
...
// Object型に ID: contentの形でarrayを格納していく
function makeObj(array) {

  // returnのためのOBJを作っておく
  var obj = new Object(1);

  // 引数として受け取ったarrayをfor文を回す
  for (var index in array[0]) {

    // 見出しを取得
    var item = array[0][index];

    // IDだけを取り出す
    var id = takeIDWithString(item);

    // 中身を取り出す
    var content = array[1][index];
...
// 見出しからIDのみを取り出す関数
function takeIDWithString(string) {

  // 引数の文字を分割する
  var stringSplited = string.split(/\[|\]/);

  // IDとして取り出す
  var id = stringIDWithString[1];

  return id;
}

// IDからキーワードを取得する
function getValueByID(obj , keyword){
...
function sumNums(array, keyword) {
  ...
  //入っている列を取得
  var indexes = getIndexByID(array, keyword);
...
function getIndexByID(array, keyword) {
...
function getColByID(sheet, keyword) {
